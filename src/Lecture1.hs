...
subString start end str = drop startix (take (endix + 1) str) where
    startix
        | start >= 0 = start
        | otherwise  = 0
    endix
...
lowerAndGreater n list = 
    show n ++ " is greater than " ++ show ngreater ++ " elements and lower than " ++ show nlower ++ " elements"
    where
